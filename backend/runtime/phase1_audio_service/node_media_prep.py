"""RTX-side node-media prep runner (ffmpeg NVENC + GCS upload).

The H200 Phase 2-4 worker invokes this via the
``POST /tasks/node-media-prep`` endpoint. The RTX host:
...
The H200 never sees the raw clip bytes; it only receives the GCS URIs.
"""
...
class _MinimalNode:
    ...
    instantiating the full pydantic ``SemanticGraphNode`` contract on the RTX
    host to keep request parsing cheap and decoupled from Phase 2-4's schema.
    """
...
def run_node_media_prep(
    *,
    ...
    """Execute one ``POST /tasks/node-media-prep`` request on the RTX host.
    ...
    scratch_root:
        Per-host scratch directory (e.g. ``/opt/clypt-phase1/scratch``).
    ...
    # The H200 only needs the GCS URIs; strip local_path (the TemporaryDirectory
    # has been cleaned up by the time this return value is serialized).
    media = [
        {
            "node_id": d["node_id"],
            "file_uri": d["file_uri"],
