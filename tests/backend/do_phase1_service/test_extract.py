...
def test_run_speaker_binding_uses_easy_span_cascade_without_lrasd(monkeypatch):
    worker_cls = ClyptWorker._get_user_cls()
    worker = worker_cls.__new__(worker_cls)
    ...
    monkeypatch.setattr(
        worker,
        "_run_lrasd_binding",
        lambda **kwargs: pytest.fail("easy span should skip LR-ASD"),
    )
    monkeypatch.setattr(
        worker,
        "_run_speaker_binding_heuristic",
        lambda *args, **kwargs: pytest.fail("easy span should not fall back to heuristic"),
    )
    monkeypatch.setattr(
        worker,
    ...
    words = [
        {"text": "hello", "start_time_ms": 0, "end_time_ms": 180},
        {"text": "world", "start_time_ms": 180, "end_time_ms": 380},
    ]
    tracks = [
        {
            "track_id": "local-1",
    ...
    result = worker._run_speaker_binding(
        ...
        tracks=tracks,
        words=words,
        frame_to_dets={0: tracks},
        track_to_dets={"local-1": tracks},
        analysis_context={
            "audio_speaker_turns": [
                {
                    "speaker_id": "SPEAKER_00",
    ...
    assert worker._last_speaker_binding_metrics["scheduled_spans_total"] == 1
    assert worker._last_speaker_binding_metrics["easy_spans_resolved"] == 1
    assert worker._last_speaker_binding_metrics["hard_spans_routed_to_lrasd"] == 0
    assert worker._last_speaker_binding_metrics["overlap_spans_count"] == 0


def test_run_speaker_binding_routes_overlap_span_to_lrasd_even_with_easy_single_span(monkeypatch):
    worker_cls = ClyptWorker._get_user_cls()
    worker = worker_cls.__new__(worker_cls)
    worker.lrasd_model = object()
    worker.lrasd_loss_av = object()
    ...
    monkeypatch.setattr(
        ...
        "_build_speaker_binding_track_quality",
        lambda *args, **kwargs: {
            "local-1": {"track_quality": 0.92},
            "local-2": {"track_quality": 0.78},
        },
    )

    observed = {"lrasd_calls": 0}

    def _fake_rank_candidates(**kwargs):
        turn = kwargs["turn"]
        if int(turn["start_time_ms"]) == 0:
            return [
                {
                    "local_track_id": "local-1",
                    "candidate_survives": True,
        ...
        return [
            {
                "local_track_id": "local-1",
                "candidate_survives": True,
                "speech_overlap_ratio": 0.71,
                "continuity_support_score": 0.77,
                "rank_score": 0.78,
            },
            {
                "local_track_id": "local-2",
                "candidate_survives": True,
                "speech_overlap_ratio": 0.66,
                "continuity_support_score": 0.74,
                "rank_score": 0.74,
            },
        ]

    monkeypatch.setattr(worker, "_rank_lrasd_turn_candidates", _fake_rank_candidates)

    def _fake_run_lrasd_binding(**kwargs):
        observed["lrasd_calls"] += 1
        words[2]["speaker_track_id"] = "Global_Person_1"
        words[2]["speaker_tag"] = "Global_Person_1"
        words[2]["speaker_local_track_id"] = "local-2"
        words[2]["speaker_local_tag"] = "local-2"
        words[3]["speaker_track_id"] = "Global_Person_1"
        words[3]["speaker_tag"] = "Global_Person_1"
        words[3]["speaker_local_track_id"] = "local-2"
        words[3]["speaker_local_tag"] = "local-2"
    ...
    monkeypatch.setattr(
        worker,
        "_run_speaker_binding_heuristic",
        lambda *args, **kwargs: pytest.fail("mixed easy/overlap case should stay on LR-ASD path"),
    )

    words = [
        {"text": "alpha", "start_time_ms": 0, "end_time_ms": 180},
        {"text": "beta", "start_time_ms": 180, "end_time_ms": 380},
        {"text": "gamma", "start_time_ms": 420, "end_time_ms": 580},
        {"text": "delta", "start_time_ms": 580, "end_time_ms": 780},
    ]
    tracks = [
        {"track_id": "local-1", "frame_idx": 0, "x_center": 80.0, "y_center": 100.0, "width": 72.0, "height": 148.0, "confidence": 0.93},
        {"track_id": "local-2", "frame_idx": 1, "x_center": 220.0, "y_center": 102.0, "width": 70.0, "height": 144.0, "confidence": 0.89},
    ]
    ...
    result = worker._run_speaker_binding(
        ...
        tracks=tracks,
        words=words,
        frame_to_dets={0: [tracks[0]], 1: [tracks[1]]},
        track_to_dets={"local-1": [tracks[0]], "local-2": [tracks[1]]},
        analysis_context={
            "audio_speaker_turns": [
                {"speaker_id": "SPEAKER_00", "turn_id": "turn-0", "start_time_ms": 0, "end_time_ms": 400},
                {"speaker_id": "SPEAKER_00", "turn_id": "turn-1", "start_time_ms": 400, "end_time_ms": 800},
                {"speaker_id": "SPEAKER_01", "turn_id": "turn-2", "start_time_ms": 400, "end_time_ms": 800},
            ],
            "scheduled_audio_spans": [
                {
                    "span_id": "scheduled-0",
                ...
                },
                {
                    ...
                    "context_end_time_ms": 800,
                    "source_turn_ids": ["turn-1", "turn-2"],
                },
            ],
        },
        track_id_remap={"local-1": "Global_Person_0", "local-2": "Global_Person_1"},
    )

    assert observed["lrasd_calls"] == 1
    assert result == [
...
def test_run_speaker_binding_passes_only_scheduled_hard_spans_to_lrasd(monkeypatch):
    worker_cls = ClyptWorker._get_user_cls()
    worker = worker_cls.__new__(worker_cls)
    worker.lrasd_model = object()
    worker.lrasd_loss_av = object()
        ...
        return [
            {
                "local_track_id": "local-2",
                "candidate_survives": True,
                "speech_overlap_ratio": 0.74,
                "continuity_support_score": 0.78,
                "rank_score": 0.82,
            }
        ]
    ...
    result = worker._run_speaker_binding(
        ...
        words=[
            {"text": "easy", "start_time_ms": 0, "end_time_ms": 200},
            {"text": "hard", "start_time_ms": 520, "end_time_ms": 780},
        ],
        frame_to_dets={},
        track_to_dets={
            "local-1": [{"track_id": "local-1", "frame_idx": 0, "x_center": 80.0, "y_center": 100.0, "width": 72.0, "height": 148.0, "confidence": 0.93}],
            "local-2": [{"track_id": "local-2", "frame_idx": 1, "x_center": 220.0, "y_center": 102.0, "width": 70.0, "height": 144.0, "confidence": 0.89}],
        },
        analysis_context={
            "audio_speaker_turns": [
                {"speaker_id": "SPEAKER_00", "turn_id": "turn-0", "start_time_ms": 0, "end_time_ms": 400},
                {"speaker_id": "SPEAKER_01", "turn_id": "turn-1", "start_time_ms": 500, "end_time_ms": 900},
            ],
            "scheduled_audio_spans": [
                {
                    "span_id": "scheduled-0",
    ...
    assert result == [
        {"track_id": "Global_Person_0", "start_time_ms": 0, "end_time_ms": 200, "word_count": 1}
    ]
    assert len(captured["scheduled_hard_spans"]) == 1
    assert captured["scheduled_hard_spans"][0]["span_id"] == "scheduled-1"
    assert captured["scheduled_hard_spans"][0]["speaker_ids"] == ["SPEAKER_01"]
    assert captured["scheduled_hard_spans"][0]["context_start_time_ms"] == 500
    assert captured["scheduled_hard_spans"][0]["context_end_time_ms"] == 900
...
def test_finalize_builds_active_speakers_local_from_scheduled_spans(monkeypatch):
    ...
    audio_turns = [
        {"speaker_id": "SPEAKER_00", "start_time_ms": 0, "end_time_ms": 100, "exclusive": True},
        {"speaker_id": "SPEAKER_00", "start_time_ms": 105, "end_time_ms": 200, "exclusive": True},
    ]

    monkeypatch.setenv("CLYPT_BINDING_SCHEDULER_SAME_SPEAKER_GAP_MS", "10")
    ...
    def _fake_run_speaker_binding(*args, **kwargs):
        ...
        return [{"track_id": "Global_Person_0", "start_time_ms": 0, "end_time_ms": 100, "word_count": 1}]
    ...
    assert result["phase_1_audio"]["active_speakers_local"] == [
        {
            ...
            "offscreen_audio_speaker_ids": [],
            "overlap": False,
            "confidence": pytest.approx(0.92, abs=0.02),
            "decision_source": "turn_binding",
        }
    ]


def test_run_lrasd_binding_ignores_synthetic_scheduled_overlap_for_assignment(monkeypatch, tmp_path: Path):
    _, words, bindings = _run_fake_lrasd_binding_case(
        monkeypatch,
        tmp_path,
        track_specs={
        ...
        words=[{"text": "hello", "start_time_ms": 0, "end_time_ms": 1000}],
        audio_speaker_turns=[
            {"speaker_id": "SPEAKER_00", "start_time_ms": 0, "end_time_ms": 1000, "exclusive": True}
        ],
        scheduled_audio_spans=[
            {
                "span_id": "scheduled-0",
