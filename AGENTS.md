...
This project is indexed by GitNexus as **Clypt-Backend** (3549 symbols, 8939 relationships, 218 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
- **MUST verify commit scope before committing.** If your installed GitNexus CLI exposes `detect_changes`, run it; otherwise run `git status --short`, `git diff --stat`, and `git diff --check` to confirm the affected files and execution surfaces are the ones you expect.
...
4. For regressions: if `detect_changes` is available in your installed CLI, use it against `main`; otherwise use `git diff --stat main...HEAD` and `git diff --check main...HEAD` — see what your branch changed

## When Refactoring

- **Renaming**: MUST use `gitnexus_rename({symbol_name: "old", new_name: "new", dry_run: true})` first. Review the preview — graph edits are safe, text_search edits need manual review. Then run with `dry_run: false`.
- **Extracting/Splitting**: MUST run `gitnexus_context({name: "target"})` to see all incoming/outgoing refs, then `gitnexus_impact({target: "target", direction: "upstream"})` to find all external callers before moving code.
- After any refactor: verify only expected files changed. Prefer `detect_changes` when your installed GitNexus CLI exposes it; otherwise use `git status --short`, `git diff --stat`, and `git diff --check`.
...
- NEVER commit changes without checking affected scope. Prefer GitNexus `detect_changes` when available; otherwise use `git status --short`, `git diff --stat`, and `git diff --check`.
...
| `detect_changes` | Pre-commit scope check when available in your installed CLI | `gitnexus_detect_changes({scope: "staged"})` |
...
3. Commit-scope verification confirms changes match expected scope (`detect_changes` when available; otherwise `git status --short`, `git diff --stat`, and `git diff --check`)
