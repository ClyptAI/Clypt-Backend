...
- Deterministic grounding is now served by a DO GPU extraction service, not managed GCP extraction APIs
...
|---|---|---|
| **SOTA Deterministic Grounding** | Produces dense multimodal ledgers using Parakeet + YOLO11/BoT-SORT + TalkNet | Phase 1 (DO worker) |
...
2. `backend/pipeline/phase_1_do_pipeline.py` submits the URL to the DO Phase 1 service and receives extraction JSON.
...
1. **DO extraction output** proving deterministic grounding from SOTA models.
