...
Full Phase 1–4 pipeline is **0.64× real-time** on a 6.5-min clip with all Vertex AI optimizations active.
...
GOOGLE_CLOUD_LOCATION=global              # Priority PayGo only works via the global endpoint
...
VERTEX_FLASH_MODEL=gemini-3-flash-preview # Dynamic meta-prompt generation (Phase 4). REQUIRED.
VERTEX_EMBEDDING_MODEL=gemini-embedding-2-preview
GCS_BUCKET=clypt-storage-v3
```

**`VERTEX_FLASH_MODEL` is required** — missing it crashes Phase 4 meta-prompt generation with a `KeyError`. The Flash model is used only for dynamic meta-prompt generation; Gemini Pro handles all other Phase 2–4 calls.
...
When running `pip install -r requirements-do-phase1.txt` on a fresh droplet, `youtokentome` may fail to build because `Cython` is not in the build environment. Fix:
...
### Priority PayGo headers only work via the global Vertex endpoint

Priority PayGo (`X-Vertex-AI-LLM-Shared-Request-Type: priority`, `X-Vertex-AI-LLM-Request-Type: shared`) is only honored when the Vertex AI generation client uses the `global` location (`GOOGLE_CLOUD_LOCATION=global`, `VERTEX_GEMINI_LOCATION=global`).
...
Longer videos (200+ speaker turns) generate more concurrent Gemini Pro calls in Phase 3 (local edge batches). These can exhaust the Vertex AI RPM/TPM limit within the rate window. The SDK's tenacity retry exhausts and the process crashes.

**Current behavior:** hard crash at Phase 3 local-edge batches if rate limit is hit.

**Workaround:** rerun with a new job ID. Retry-with-backoff is not yet implemented. Reducing `phase3_target_batch_count` in `V31Config` reduces concurrency and may avoid the limit.
