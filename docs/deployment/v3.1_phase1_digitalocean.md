...
python - <<'PY'
import os
for k in ["CLYPT_PHASE1_CACHE_HOME","XDG_CACHE_HOME","TORCH_HOME","HF_HOME","MODELSCOPE_CACHE"]:
...
Set `VERTEX_FLASH_MODEL` explicitly for reproducibility. The Flash model is used only for dynamic meta-prompt generation; Gemini Pro handles all other Phase 2–4 calls.
