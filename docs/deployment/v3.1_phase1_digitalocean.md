...
- **Execution:** visual extraction + ASR run **concurrently** (vLLM is HTTP, not CUDA); forced alignment / emotion2vec+ / YAMNet remain serial after both complete
...
| mrbeastflagrant.mp4 | 392.9s | 104 | 179.2s (3.0 min) | — | **Full Phase 1** (visual ∥ ASR + NFA + emotion2vec+ + YAMNet) |
