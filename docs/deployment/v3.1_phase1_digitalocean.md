...
7. Stops Phase 1 API/worker, then prewarms emotion2vec+ and NFA into persistent host cache (`/opt/clypt-phase1/.cache`) with timeout+retries, so the first real Phase 1 job does not block on large downloads
...
14. Restarts the Phase 1 API + worker
...
VIBEVOICE_OUTPUT_MODE=turns
VIBEVOICE_WORD_TURN_GAP_MS=900
...
`VIBEVOICE_OUTPUT_MODE=words` is experimental and bypasses NeMo forced alignment. In that mode, the runtime synthesizes turns from word-level output using `VIBEVOICE_WORD_TURN_GAP_MS`.
...
- emotion2vec+ model (`iic/emotion2vec_plus_large`, ~1.81GB from ModelScope)
...
By default, the runtime now treats `0 words` on non-empty ASR turns as a hard error (`CLYPT_PHASE1_REQUIRE_FORCED_ALIGNMENT=1`). Set it to `0` only as a temporary debugging bypass. This guard applies to `VIBEVOICE_OUTPUT_MODE=turns`; in `words` mode, forced alignment is skipped.
