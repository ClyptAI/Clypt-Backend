...
CLYPT_PHASE1_VISUAL_DECODE_QUEUE_DEPTH=2
CLYPT_PHASE1_VISUAL_DECODE_BUFFER_CHUNKS=3
CLYPT_PHASE1_VISUAL_DECODE_HWACCEL_DEVICE=cuda:0
CLYPT_PHASE1_VISUAL_DECODE_HW_DEVICE_INDEX=0
CLYPT_PHASE1_VISUAL_BENCHMARK_BATCH_SIZES=16,24,32
...
| `CLYPT_PHASE1_VISUAL_DECODE_QUEUE_DEPTH` | `2` | Bounded decode/infer overlap queue depth for the Phase1 visual pipeline. |
| `CLYPT_PHASE1_VISUAL_DECODE_BUFFER_CHUNKS` | `3` | TorchAudio StreamReader buffer chunk count before older decoded chunks are dropped. |
| `CLYPT_PHASE1_VISUAL_DECODE_HWACCEL_DEVICE` | `cuda:0` | CUDA target used by NVDEC StreamReader for direct-CUDA frame tensors. |
| `CLYPT_PHASE1_VISUAL_DECODE_HW_DEVICE_INDEX` | `0` | FFmpeg/NVDEC GPU index passed into the hardware decoder. |
| `CLYPT_PHASE1_VISUAL_BENCHMARK_BATCH_SIZES` | `16,24,32` | Static TensorRT batch sizes used by the visual-only benchmark helper. |
