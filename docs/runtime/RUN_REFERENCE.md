...
- Current code paths to validate against these historical baselines:
  ...
  - SGLang Qwen on `:8001`
  - Phase 1 ASR selectable between local vLLM and the combined Cloud Run L4 service
  - node-media prep selectable between local extraction and the combined Cloud Run L4 service
- No fresh, code-aligned benchmark has been written into this file yet for the new combined L4 offload path.
