...
- **Follow-up fix:** Live RF-DETR prewarm exposed that `rfdetr 1.6.5.post0` rewrites Nano/Small `positional_encoding_size` when `resolution=640` is passed, causing checkpoint load to fail with `size mismatch for backbone.0.encoder.encoder.embeddings.position_embeddings`. The AMD RF-DETR loader now preserves the configured `640` runtime resolution while passing the checkpoint-native PE grid (`24` for Nano, `32` for Small), allowing the pretrained checkpoint to load and interpolate at inference time instead of retuning Phase1 visual shape.
