...
- **Subsystem:** Modal media L40S Phase6 renderer (`backend/runtime/phase6_render.py`, `scripts/modal/media_worker_app.py`)
- **Environment:** DigitalOcean MI300X host `clypt-phase26-mi300x-atl1` at `134.199.205.62`, Modal profile `rithuuu`, stuck E2E render call `fc-01KR0TT1FB5VBH38Z7NDGGBVCV`, render-only smoke call `fc-01KR0WMQYA501CJ4P4BMZSNF0Z`
- **Symptom / Error signature:** The measured E2E run advanced through Phase2-4 and the visual hard join, then hung indefinitely in Phase6. Inside the active Modal media GPU container, `ffmpeg` stayed asleep with a `48-byte` output MP4 stub and no encoder progress. A one-second manual repro in the same container also hung until killed by `timeout`, again leaving only a `48-byte` output file.
- **Root cause:** The active Phase6 auto-follow implementation used FFmpeg `sendcmd` to change crop `x/y/w/h` over time. Live isolation proved the wedge was specifically tied to dynamic crop `w/h`: static crop worked, static crop plus subtitles worked, dynamic `x/y` only worked, and dynamic `w/h` only hung. In other words, the problem was not libass, not NVENC by itself, and not the HTTP poll/result path. It was runtime crop-size reconfiguration in a single ffmpeg render pass.
- **Fix applied:** Changed the live Modal renderer to freeze one fixed 9:16 crop size per clip, chosen as the largest fixed size that fits every selected-tracklet keyframe in that clip, and to drive dynamic `x/y` only through `sendcmd`. When bbox/pose data is present, `x/y` are re-derived from `bbox_xyxy` + `anchor_x` for the fixed crop size before interpolation, so the crop still follows the subject horizontally while respecting inside-person bounds. Stopped and redeployed the Modal media app to flush wedged in-flight calls from the single-worker pool.
- **Verification evidence:** Focused renderer tests passed locally after the patch: `./.venv/bin/python -m pytest tests/backend/runtime/test_phase6_render.py -q` (`7 passed`), `./.venv/bin/python -m pytest tests/scripts/test_modal_render_video_app.py -q` (`3 passed`), and `./.venv/bin/python -m pytest tests/backend/runtime -q` (`77 passed`). Live render-only smoke on the already-generated Phase6 artifacts succeeded under call `fc-01KR0WMQYA501CJ4P4BMZSNF0Z` with `total_ms=10865.8` and output `gs://clypt-storage-v3/phase14/run_64qbe35s0ek_20260507_085544_phase6_smoke/render_outputs/sg_0002_cand_01.mp4`. Clean end-to-end rerun `run_64qbe35s0ek_20260507_094450_clean` then completed successfully with Phase1 visual submit `226.4 ms`, Scribe `80346.1 ms`, adapter `17.7 ms`, Phase1 total `85.9 s`, Phase2 `82576.8 ms`, Phase3 `50.7 ms`, Phase4 `14159.7 ms`, visual join `142969.6 ms`, Phase6 `79149.3 ms`, and total Phase24 `407820.5 ms`.
- **Follow-up guardrails:** Do not re-enable dynamic crop `w/h` in a single FFmpeg/NVENC render pass without a fresh live proof. If future product work truly needs size changes within a clip, solve it with a different render architecture such as per-run segment renders plus concat, or another renderer that can safely tolerate dynamic resolution changes.

## 2026-05-07 - TensorRT visual postprocess over-suppressed adjacent people and Phase6 clip-wide crop sizing caused face-zoom clips

- **Date/Time (UTC):** 2026-05-07 23:18 UTC
- **Subsystem:** Phase1 TensorRT RF-DETR-Seg postprocess + Phase6 render worker (`backend/phase1_runtime/tensorrt_detector.py`, `backend/runtime/phase6_render.py`)
- **Environment:** AMD-refactor local code review against clean run `run_64qbe35s0ek_20260507_094450_clean`
- **Symptom / Error signature:** In longform debug overlays, adjacent seated people could appear as separate boxes and then collapse into one giant surviving `RAW` box when overlap increased. In rendered shorts, an unrelated late narrow tracklet could collapse the whole clip to a face-only crop because the live renderer used one fixed crop size for the entire clip.
- **Root cause:** The active TensorRT detector path diverged from RF-DETR upstream semantics by applying a custom hard box-IoU NMS after thresholding and `person` filtering. That made overlapping adjacent people vulnerable to collapse into one surviving detection. Separately, the live Phase6 renderer used the minimum `w/h` across every crop keyframe in the clip to choose one fixed crop size for the whole clip, so one tiny late run could shrink earlier runs into unusable face crops.
- **Fix applied:** Removed the extra hard box-IoU NMS from the active TensorRT RF-DETR-Seg postprocess path so the detector now decodes logits, thresholds, filters to `person`, and retains the surviving person queries/masks directly. Replaced the clip-wide fixed-size Phase6 render workaround with a per-run/per-tracklet render path: each run is rendered as its own fixed-size cropped video piece with run-local `sendcmd` `x/y` motion, the pieces are stitched back into one clip, and subtitles are applied in a final pass over the stitched clip.
- **Verification evidence:** Local focused and impacted suites passed after the patch: `./.venv/bin/python -m pytest tests/backend/phase1_runtime/test_rfdetr_visual_modules.py -q` (`28 passed`), `./.venv/bin/python -m pytest tests/backend/runtime/test_phase6_render.py -q` (`7 passed`), `./.venv/bin/python -m pytest tests/backend/phase1_runtime/test_media_and_visual.py -q` (`6 passed`), `./.venv/bin/python -m pytest tests/backend/pipeline/render/test_phase6_metadata_and_compiler.py -q` (`14 passed`), `./.venv/bin/python -m pytest tests/scripts/test_modal_visual_extract_app.py -q` (`8 passed`), and `./.venv/bin/python -m pytest tests/backend/runtime -q` (`77 passed`).
- **Follow-up guardrails:** Do not reintroduce a custom hard box-IoU NMS stage on the active TensorRT RF-DETR-Seg path without an explicit need and approval. Do not revert to one fixed crop size per whole clip; if the renderer needs more within-run sophistication later, extend the per-run piecewise render path instead of collapsing the entire clip again.

## 2026-05-07 - YOLO pose validator blended adjacent subjects by averaging all pose instances in one crop

- **Date/Time (UTC):** 2026-05-07 21:20 UTC
- **Subsystem:** Phase1 visual pose validation / Phase6 crop-anchor inputs (`backend/phase1_runtime/pose_subject_validator.py`, `backend/phase1_runtime/masks.py`)
- **Environment:** AMD-refactor visual pipeline using RF-DETR-Seg low-resolution `mask_ref` sidecars and YOLO11s-pose subject validation
- **Symptom / Error signature:** In couch and group shots, the selected Phase6 crop could land between people or cut off the chosen subject's head even though the RF-DETR track box itself looked correct in the longform debug overlay. The failure reproduced most clearly in `sg_0005_cand_01.mp4`, where the crop planner received an anchor near the midpoint between adjacent seated subjects instead of the target subject's actual head position.
- **Root cause:** The pose validator averaged head and shoulder keypoints across every YOLO pose instance returned inside the padded RF-DETR crop. When a crop contained more than one person, that produced blended `head_center_xy` / `upper_torso_anchor_xy` values that did not belong to any single tracked subject. Because the blended anchor was still inside the RF-DETR bbox, downstream crop clamping behaved as designed and therefore preserved the bad framing rather than correcting it.
- **Fix applied:** Added `MaskArtifactReader` to decode the stored low-resolution RF-DETR-Seg mask sidecar for each sampled tracked row. The validator now scores each YOLO pose instance by how its confident keypoints land inside the tracked subject's mask, selects one winning pose instance per sample, computes anchors from that one instance only, and uses the previous accepted anchor only as a tie-break. The old multi-instance averaging path was removed from production flow; the remaining direct helper now hard-fails if called with more than one pose instance.
- **Verification evidence:** Added regression coverage in `tests/backend/phase1_runtime/test_pose_subject_validator.py` that constructs a two-person pose crop plus a right-half-only RF-DETR mask and verifies the validator selects the right-side subject instead of averaging both subjects. Focused and impacted suites passed locally after the patch: `./.venv/bin/python -m pytest tests/backend/phase1_runtime/test_pose_subject_validator.py -q` (`6 passed`), `./.venv/bin/python -m pytest tests/backend/phase1_runtime/test_media_and_visual.py -q` (`6 passed`), `./.venv/bin/python -m pytest tests/backend/phase1_runtime/test_rfdetr_visual_modules.py -q` (`28 passed`), `./.venv/bin/python -m pytest tests/backend/pipeline/test_timeline_phase1.py -q` (`5 passed`), `./.venv/bin/python -m pytest tests/backend/pipeline/render/test_phase6_metadata_and_compiler.py -q` (`14 passed`), and `./.venv/bin/python -m pytest tests/scripts/test_modal_visual_extract_app.py -q` (`8 passed`).
- **Follow-up guardrails:** Do not reintroduce any pose-anchor computation that pools keypoints across multiple YOLO pose detections in one crop. If future work needs richer multi-person disambiguation, layer it on top of the per-track mask association rather than falling back to global crop-level averaging.

## 2026-05-07 - Modal visual result transport timed out on oversized inline payload and Phase26 retry falsely short-circuited success

- **Date/Time (UTC):** 2026-05-07 08:20 UTC
- **Subsystem:** Modal visual result transport + Phase26 resume path (`scripts/modal/visual_extract_app.py`, `backend/providers/visual_extract_client.py`, `backend/runtime/phase14_live.py`)
- **Environment:** DigitalOcean MI300X host `clypt-phase26-mi300x-atl1` at `134.199.205.62`, Modal profile `rithuuu`, measured visual call `fc-01KR0Q0TGXM7J0FPN4MJT40FAB`, queue task `6b9945b0-f763-445d-ac81-1c4037bea587`
- **Symptom / Error signature:** The Modal GPU function completed, but Phase26 failed the visual hard join with `TimeoutError: The read operation timed out` while polling the visual result. Direct Modal inspection showed the call had already succeeded and the returned object contained `payload_size_bytes: 370696722`. A subsequent retry then incorrectly marked the run terminal with `final_status=PHASE24_DONE` without actually completing the visual join.
- **Root cause:** After the mask-sidecar refactor, the worker still returned the full validated `phase1_visual` JSON inline over the HTTP result surface. On a real video this object was still about `370 MB`, which caused the poll/read path to time out even though the GPU job itself had finished successfully. Separately, the resume logic in `V31LivePhase14Runner` treated persisted Phase4 metrics as sufficient for immediate success, so a retry after a visual-join failure could skip the actual join and falsely terminate the run.
- **Fix applied:** Changed the Modal visual worker to upload the full validated visual payload as compressed `phase14/<run_id>/visual/phase1_visual.json.gz` and return only pointer metadata (`phase1_visual_gcs_uri`, `phase1_visual_encoding=json_gzip_v1`) from the result surface. Added host-side artifact hydration in `RemoteVisualExtractClient` so Phase26 still receives a normal `phase1_visual` object after downloading/decompressing the GCS artifact. Removed the Phase4-success short-circuit in `V31LivePhase14Runner`; resume after visual-join failure now reloads persisted Phase2/Phase3 artifacts, reruns Phase4, and retries the visual join instead of marking terminal success.
- **Verification evidence:** Focused transport/resume tests passed: `./.venv/bin/python -m pytest tests/backend/providers/test_visual_extract_client.py tests/scripts/test_modal_visual_extract_app.py tests/backend/runtime/test_pending_visual_contract.py -q` (`14 passed`). Wider targeted regression tests passed: `./.venv/bin/python -m pytest tests/backend/providers/test_visual_extract_client.py tests/scripts/test_modal_visual_extract_app.py tests/backend/runtime/test_phase14_live_resume.py tests/backend/runtime/test_phase24_worker_app.py tests/backend/phase1_runtime/test_runner.py tests/backend/runtime/test_pending_visual_contract.py -q` (`35 passed`). Full runtime suite passed after the resume change: `./.venv/bin/python -m pytest tests/backend/runtime -q` (`76 passed`). Live redeploy and rerun verification must still confirm that the poll path no longer times out on the measured E2E flow.
- **Follow-up guardrails:** Keep long-lived visual payloads off the Modal poll/result surface. Submit/poll must carry bounded metadata only; large artifacts belong in GCS with explicit hydration on the host side. Phase26 resume logic must never infer final success from persisted phase metrics alone when a downstream hard join was still pending or failed.

## 2026-05-07 - Modal visual timing run included one-time YOLO pose TensorRT setup

- **Date/Time (UTC):** 2026-05-07 06:40 UTC
- **Subsystem:** Modal visual L40S warmup/readiness (`requirements-modal-visual-l40s.txt`, `backend/phase1_runtime/pose_subject_validator.py`, visual deploy runbook)
- **Environment:** Modal profile `rithuuu`, app `clypt-visual-l40s`, invalid measured E2E job `job_64b2cd56ab9f4638a11fb71112609f81`, visual call `fc-01KR0J9AGR83S4TSRKYXAPKY2C`
- **Symptom / Error signature:** The first post-patch E2E visual call reached the YOLO11s-pose validation stage and then downloaded `yolo11s-pose.pt`, auto-installed missing Ultralytics export dependencies (`onnxslim`, `onnxruntime-gpu`), exported ONNX, and started TensorRT engine build inside the run. This made the run invalid for Phase1-6 timing because it included one-time setup.
- **Root cause:** The earlier synthetic warmup clip had no people, so it exercised RF-DETR-Seg service startup but did not force sampled YOLO pose validation. Separately, `requirements-modal-visual-l40s.txt` omitted the YOLO TensorRT export helper dependencies, so Ultralytics attempted runtime package installation during the first person-containing call.
- **Fix applied:** Added `onnxslim>=0.1.71` and `onnxruntime-gpu>=1.25.1` to `requirements-modal-visual-l40s.txt`, redeployed the Modal visual image, cancelled the contaminated visual call, and changed the operational warmup rule: a timing-valid visual warmup must use a person-containing clip and must complete successfully before measured E2E submission.
- **Verification evidence:** Focused local visual/modal tests passed after the dependency patch (`46 passed, 3 warnings`). Live warmup is in progress under visual call `fc-01KR0K8XVAMGCDQ3FNE86E2RXG`; measured E2E timing must wait until that warmup returns success.
- **Follow-up guardrails:** Treat visual readiness as a real workload readiness gate, not just `/health`. `/health` only proves the ASGI surface is up; it does not prove TensorRT engines are built or that the pose validator has been initialized.

## 2026-05-07 - RF-DETR-Seg visual worker hung on full-frame Python mask serialization

- **Date/Time (UTC):** 2026-05-07 05:45 UTC
- **Subsystem:** Modal visual L40S RF-DETR-Seg mask retention (`backend/phase1_runtime/tensorrt_detector.py`, `backend/phase1_runtime/visual.py`, `backend/phase1_runtime/tracker_runtime.py`)
- **Environment:** Modal profile `rithuuu`, app `clypt-visual-l40s`, E2E job `job_cfc04320519f483ab5af544c33638033`, visual call `fc-01KR0DJEB6H7RT30SRK751RRST`
- **Symptom / Error signature:** After the Seg Nano PE-grid and shape fixes, the visual job successfully exported ONNX with mask output tensors, but the result endpoint stayed `202 Accepted` for more than 25 minutes. The Modal container showed Python and ffmpeg alive, about `6.5 GiB` VRAM allocated, and `0%` sampled GPU utilization. No normal `[visual] RF-DETR-Seg ... frames processed` progress line appeared before the run was manually cancelled.
- **Root cause:** The first RF-DETR-Seg mask-retention implementation resized every selected low-resolution instance mask to full source-frame dimensions and then encoded each full-frame boolean mask with a pure Python row-major RLE loop. It did this for raw detections and again after ByteTrack association, then duplicated mask data through raw detections, tracks, person detections, and tracklet geometry. That moved the hot path from GPU TensorRT inference to CPU-bound mask materialization and oversized JSON payload construction.
- **Fix applied:** Replaced inline full-frame `mask_rle` emission with a compressed low-resolution `.npz` sidecar artifact per visual job. JSON rows now carry lightweight `mask_ref` pointers using `lowres_mask_ref_v1`; ByteTrack remains box-only and mask refs are associated to tracked rows by same-frame IoU after identity assignment. Added visual metrics for TensorRT inference, output copy, postprocess, mask extraction, mask artifact write time/bytes, ByteTrack time, mask association time, payload size, and effective FPS. Modal uploads the sidecar to GCS before returning the Phase1 visual payload.
- **Verification evidence:** Local focused tests passed for the new mask-ref contract and Modal upload path. Live verification is pending a warmed Modal redeploy and a fresh E2E run; engine build/warmup must be completed before measuring run timing.
- **Follow-up guardrails:** Do not emit full source-frame mask blobs in Phase1 visual JSON. Future mask consumers should resolve `mask_ref` sidecars at the point of use and only upsample masks inside the render/caption-planning worker when pixel-level operations are actually required.

## 2026-05-07 - RF-DETR-Seg Nano TensorRT export used detection-only PE grid and invalid shape

- **Date/Time (UTC):** 2026-05-07 05:01 UTC
- **Subsystem:** Modal visual L40S RF-DETR-Seg TensorRT export (`backend/phase1_runtime/tensorrt_detector.py`)
- **Environment:** Modal profile `rithuuu`, app `clypt-visual-l40s`, E2E job `job_d4acf7bf0bd142b2896dec9704ae5237`, visual call `fc-01KR0CY8CWVXXKPQJVMXF98KDQ`
- **Symptom / Error signature:** First live RF-DETR-Seg job downloaded `rf-detr-seg-nano.pt`, then failed before TensorRT engine build with `RuntimeError: size mismatch for backbone.0.encoder.encoder.embeddings.position_embeddings: copying a param with shape torch.Size([1, 677, 384]) from checkpoint, the shape in current model is torch.Size([1, 577, 384])`. After fixing the PE grid, the next live job failed ONNX export with `ValueError: shape must have both dimensions divisible by 12 (patch_size=12 * num_windows=1), got (640, 640)`.
- **Root cause:** The Modal visual path carried over detection-only RF-DETR Nano assumptions. Detection-only Nano used positional-encoding grid `24`, which gives `24 * 24 + 1 = 577` rows, but `RFDETRSegNano` requires grid `26` for its checkpoint (`26 * 26 + 1 = 677` rows). Separately, Seg Nano uses patch size `12`, so the previous `640x640` visual shape is invalid.
- **Fix applied:** Changed the Seg Nano checkpoint PE grid to `26`, changed the active visual shape to `648x648` (`54 * 12`), updated focused TensorRT/config tests, and updated active env/docs.
- **Verification evidence:** Pending in the same live smoke: redeploy Modal visual, submit a fresh E2E job for `https://youtu.be/64qBE35S0ek?si=bul2StVGVzUE8EL6`, and confirm the Seg Nano TensorRT engine builds and emits masks.
- **Follow-up guardrails:** Do not assume detection-only RF-DETR checkpoint geometry applies to RF-DETR-Seg variants. For future model swaps, verify the checkpoint `position_embeddings` row count against `positional_encoding_size * positional_encoding_size + 1` before first live deploy.
...
- **Subsystem:** Phase6 render plan / Phase5-less auto-follow crop selection (`backend/pipeline/render/compiler.py`, `backend/runtime/phase6_render.py`, Modal media L40S)
- **Environment:** AMD-refactor topology, Modal apps `clypt-visual-l40s` and `clypt-media-l40s`, render replay using `job_visual_streamfix_mrbeast_20260503_190000` visual payload
- **Symptom / Error signature:** Rendered outputs were technically valid `1080x1920` clips and smoother than the previous step-function crop path, but human review still judged them terrible: tracking/subject selection was unreliable and crop movement was not smooth enough for product-quality export.
- **Root cause:** The current Phase5-less fallback depends on automatic shot-level tracklet choice plus bbox/pose-derived crop paths. Even after RF-DETR TensorRT decode, ByteTrack cut reset, pose eligibility filtering, and FFmpeg command-file crop motion fixes, the selected tracklets and crop path are still not robust enough to replace manual Phase5 grounding. A replay from an older visual payload also used bbox upper-third fallback anchors rather than fresh persisted pose anchors, further limiting quality.
- **Fix applied:** No production-quality visual fix accepted yet. Documentation now marks Phase5-less auto-follow render as experimental only, not an accepted baseline. Modal render infrastructure remains patched to avoid black bars, double captions, and FFmpeg inline-expression failures.
- **Verification evidence:** Rendered clips were downloaded to `/Users/rithvik/Downloads/clypt_inside_person_smooth_rendered_clips_20260504` and probed as `1080x1920`, but were rejected by visual review for poor tracking and insufficiently smooth crop motion. Both Modal apps were stopped after testing to avoid overnight GPU spend.
- **Follow-up guardrails:** Do not call auto-follow renders production-ready based on successful MP4 generation alone. The next quality pass should debug tracklet selection, subject continuity, pose-anchor persistence, crop-path smoothing, and shot-level subject assignment against human-reviewed overlays before enabling Phase5-less exports.

## 2026-05-04 - Modal Phase6 smooth crop path exceeded FFmpeg inline expression limits

- **Date/Time (UTC):** 2026-05-04 03:23-03:37 UTC
- **Subsystem:** Phase6 render plan + Modal L40S ffmpeg renderer (`backend/pipeline/render/compiler.py`, `backend/runtime/phase6_render.py`)
- **Environment:** AMD-refactor topology, Modal app `clypt-media-l40s`, render-only replay using `job_visual_streamfix_mrbeast_20260503_190000` visual payload and canonical test-bank video `mrbeastflagrant.mp4`
- **Symptom / Error signature:** Modal render submissions failed immediately on the first clip with `subprocess.CalledProcessError` from FFmpeg; the logged `-vf` argument contained hundreds of nested `if(between(t...))` clauses for `crop=x:y` interpolation and Modal reported the exception as too large.
- **Root cause:** The initial smooth inside-person crop implementation embedded the entire crop keyframe path directly in the FFmpeg filter string. Longer clips produced very large nested expressions that exceeded practical FFmpeg/Modal command handling limits.
- **Fix applied:** Reduced render-plan crop keyframe density and duplicate emissions, then replaced the inline keyframe expression path in `run_phase6_render()` with a temporary FFmpeg `sendcmd` file that drives `crop@follow` x/y updates per output frame. The filter string now stays compact while the crop path remains smooth.
- **Verification evidence:** Focused render tests passed locally (`16 passed, 1 warning`); `compileall` passed for the render compiler/runtime; redeployed Modal media; reran render call `fc-01KQRGWHF27MF1R0N9XW57245M`, which succeeded and produced four clips under `gs://clypt-storage-v3/phase14/job_visual_streamfix_inside_person_render_20260504/render_outputs/`. Downloaded outputs to `/Users/rithvik/Downloads/clypt_inside_person_smooth_rendered_clips_20260504`; all four probed as `1080x1920`.
- **Follow-up guardrails:** Do not encode long motion paths as nested FFmpeg expressions. For dynamic crop movement, prefer command files or another data-driven filter control surface so command length does not scale with clip duration.

## 2026-05-04 - RF-DETR TensorRT boxes lagged by one inference batch and leaked across cuts

- **Date/Time (UTC):** 2026-05-04 02:05 UTC
- **Subsystem:** Modal visual L40S TensorRT RF-DETR + ByteTrack (`backend/phase1_runtime/tensorrt_detector.py`, `backend/phase1_runtime/visual.py`, `backend/phase1_runtime/tracker_runtime.py`)
- **Environment:** AMD-refactor topology, Modal app `clypt-visual-l40s`, canonical test-bank video `mrbeastflagrant.mp4`
- **Symptom / Error signature:** The longform tracklet debug overlay showed tracked boxes visually delayed by about `500-700 ms`; around the hard cut near `266.516 s` (`frame 6390` at `23.976 fps`), boxes from the previous wide shot appeared on the new close-up shot for roughly one RF-DETR batch. Post-hoc inspection of `job_visual_pose_mrbeast_20260503_181049` showed stale wide-shot boxes persisted through frames `6390-6405` and jumped to the close-up geometry at `6406`, matching the configured batch size `16`.
- **Root cause:** PyTorch preprocessing and `input_binding["buffer"].copy_(input_tensor)` ran on PyTorch's current CUDA stream, while TensorRT `execute_async_v3(...)` ran on a separate custom stream. Without an explicit stream dependency, TensorRT could consume the previous batch's input buffer. Separately, ByteTrack state was continuous across camera cuts; post-track shot splitting changed IDs defensively but did not stop tracker state from influencing the first frames after a cut.
- **Fix applied:** Added an ordered CUDA stream handoff before TensorRT execution (`self._stream.wait_stream(torch.cuda.current_stream())`), reset ByteTrack at hard shot-cut frame boundaries before updating the tracker for the next shot, kept post-track shot splitting as a defensive cleanup layer, and emitted `raw_person_detections` so debug overlays can compare raw RF-DETR detections against tracked boxes.
- **Verification evidence:** Added regressions for TensorRT stream ordering and shot-boundary tracker resets; focused visual/runtime tests passed locally (`33 passed, 1 warning`). Redeployed Modal visual and ran visual-only job `job_visual_streamfix_mrbeast_20260503_190000` / call `fc-01KQRB0YAY8AC4RY78NMSPP6FX`: output had `29,881` tracked rows, `29,881` raw RF-DETR rows, `60` shots, `59` tracker resets, `239` pose-validated tracklets, and `234.1` effective fps. At frame `6390`, raw and tracked boxes both moved immediately to close-up geometry (`[4.7, 96.5, 1586.2, 1072.1]`) instead of carrying the previous shot. Generated validation overlay at `/Users/rithvik/Downloads/clypt_job_visual_streamfix_tracklet_debug/mrbeastflagrant_raw_vs_tracked_streamfix.mp4`.
- **Follow-up guardrails:** Any TensorRT path that mixes PyTorch-side preprocessing with custom CUDA streams must explicitly order the streams before enqueue. Keep raw-vs-tracked overlays available for timing regressions; if raw boxes are aligned but tracked boxes lag, inspect ByteTrack smoothing/state, and if both lag, inspect decode or stream ordering.

## 2026-05-04 - RF-DETR TensorRT boxes decoded as the wrong coordinate format

- **Date/Time (UTC):** 2026-05-04 00:13 UTC
- **Subsystem:** Modal visual L40S TensorRT RF-DETR + ByteTrack (`backend/phase1_runtime/tensorrt_detector.py`)
- **Environment:** AMD-refactor topology, Modal app `clypt-visual-l40s`, canonical test-bank video `mrbeastflagrant.mp4`
- **Symptom / Error signature:** Shot boundaries were accurate, but rendered tracking overlays showed boxes nowhere near people. The visual artifact from `job_00bc0fa2d5e44a7ca6385e4af4ae3df8` had `43,271` track records where max `x2` was about `3.05`, max `y2` was about `1.81`, and `36,492` records were degenerate after interpretation.
- **Root cause:** The TensorRT RF-DETR path treated exported ONNX box outputs as detector-resolution `xyxy`, multiplying by `source_width / 640` and `source_height / 640`. RF-DETR's exported head emits normalized `cx, cy, w, h`, so ByteTrack received tiny/invalid boxes. After fixing the coordinate decode, a second issue remained: overlapping duplicate person queries were fed directly into ByteTrack without NMS, creating duplicate same-track rows in some frames.
- **Fix applied:** Decode TensorRT RF-DETR boxes from normalized `cxcywh` to source-frame `xyxy` pixels before constructing `sv.Detections`, clamp to source dimensions, and apply person-class NMS (`iou_threshold=0.5`) before ByteTrack.
- **Verification evidence:** Added regression tests for normalized box decode and pre-tracker NMS; `tests/backend/phase1_runtime` passed locally (`60 passed, 1 warning`). Redeployed Modal visual and ran visual-only job `job_visual_fix_nms_mrbeast_20260503` / call `fc-01KQR4Z0C7K9FQ948W2D63QW4X`: output had `34,869` track rows, `60` shots, zero degenerate boxes, all `34,869` rows with `>20x20` boxes, and only `188 / 9,403` frames with duplicate same-ID rows. Generated the longform debug overlay at `/Users/rithvik/Downloads/clypt_job_visual_fix_nms_tracklet_debug/mrbeastflagrant_tracklets_debug_fixed.mp4`.
- **Follow-up guardrails:** Any future TensorRT export/runtime change must verify raw box format against a known frame before ByteTrack. Tracklet debug overlays should report degenerate-box counts and duplicate same-track-per-frame counts as acceptance signals.

## 2026-05-03 - Phase6 rendered clips were padded and double-captioned

- **Date/Time (UTC):** 2026-05-03 23:32 UTC
- **Subsystem:** Phase6 render plan + Modal L40S ffmpeg renderer (`backend/pipeline/render/*`, `backend/runtime/phase6_render.py`)
- **Environment:** AMD-refactor topology, Phase26 MI300X orchestrator/Qwen host plus Modal L40S media worker, live E2E run `job_e9f3565f0d2049b7827a54021a89867f`
- **Symptom / Error signature:** Final clips rendered as 16:9 video padded into a 1080x1920 canvas with large black top/bottom bars; captions appeared as stacked/doubled overlays, used yellow active words, and could run off-screen.
- **Root cause:** The Modal render worker explicitly used `scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920...`, which preserves the source aspect and letterboxes instead of cropping to fill. The ASS compiler emitted a full base caption line for the whole segment and then a second highlighted line for each active word, causing visible duplicate overlays. The render plan did not synthesize a Phase5-free follow target from Phase1 tracklets, so no bounding-box data reached the renderer when manual grounding was absent.
- **Fix applied:** Changed Phase6 caption generation to emit a single active karaoke line at a time, switched `karaoke_focus` active color to green with smaller safer text settings, added deterministic auto-follow tracklet selection from Phase1 `ShotTrackletIndex`/`TrackletGeometry` when no camera intent exists, serialized per-segment crop instructions into `RenderPlanClip.crop_plan`, and updated the Modal renderer to crop-to-fill 9:16 with tracklet-centered crop expressions instead of padding.
- **Verification evidence:** Added regression coverage for green single-layer ASS captions, fallback tracklet crop-plan compilation, center crop-to-fill rendering, and tracklet crop rendering. Verified locally with `17 passed` for render/runtime focused tests, `89 passed` for `tests/backend/pipeline`, and `73 passed` for `tests/backend/runtime`. Redeployed Modal media, synced/restarted MI300X Python services, and ran fresh E2E `job_00bc0fa2d5e44a7ca6385e4af4ae3df8`: Phase6 succeeded in `57,495.9 ms`, all 12 rendered MP4s were `1080x1920`, render plans contained `tracklet_follow_9x16` crop segments, and sampled ASS files had green `&H0000FF25` active words with no `Dialogue: 0` duplicate layer.
- **Follow-up guardrails:** Do not reintroduce `force_original_aspect_ratio=decrease` + `pad` for final shorts. Phase5-less E2E tests must still consume Phase1 tracklets through auto-follow crop plans so render output exercises the same vertical framing path as manual grounding.
...
## 2026-05-03 - Phase26 MI300X deploy env failed on unquoted acceptance profiles

- **Date/Time (UTC):** 2026-05-03 20:05 UTC
- **Subsystem:** Phase26 MI300X deploy env (`docs/runtime/known-good-phase26-mi300x.env`, `scripts/do_phase26/deploy_phase26_mi300x_services.sh`)
- **Environment:** DigitalOcean `clypt-phase26-mi300x-atl1` (`129.212.188.235`), ROCm 7.2 MI300X DevCloud image
- **Symptom / Error signature:** `deploy_phase26_mi300x_services.sh` failed immediately after sourcing `/etc/clypt-phase26/phase26.env` with `/etc/clypt-phase26/phase26.env: line 29: strict_json: command not found`.
- **Root cause:** `SG_ACCEPTANCE_PROFILES` is intentionally a space-separated list consumed by the deploy script's `for profile in $SG_ACCEPTANCE_PROFILES` loop, but the known-good env recorded it without quotes. Bash treated the second word as a command while sourcing the env file.
- **Fix applied:** Quoted `SG_ACCEPTANCE_PROFILES="minimal strict_json fp8_kv scheduler_cache speculative"` in the known-good env and operator docs, then regenerated the droplet env from the corrected source.
- **Verification evidence:** The clean live MI300X deploy validated staged SGLang profiles through `minimal`, `strict_json`, `fp8_kv`, `scheduler_cache`, and `speculative`, then started the final profile. The final service listed `Qwen/Qwen3.6-35B-A3B`, passed the `LocalOpenAIQwenClient` smoke, passed the long-context JSON smoke, and the Phase26 dispatch, Phase1 API, Phase26 worker, and Phase1 worker services all reached `active`.
- **Follow-up guardrails:** Any host env value containing spaces must be shell-quoted because deploy scripts source these files directly through the shared `load_env_file` helper.
- **Follow-up fix:** The next deploy attempt exposed a second deploy-script quoting issue: the Qwen prewarm Python heredoc was unquoted while containing Python f-strings with `${path!r}` / `${HF_HOME!r}` syntax, causing Bash to fail with `bad substitution`. The prewarm block now passes `SG_MODEL`, `SG_MODEL_REVISION`, `SG_MODEL_ENV_FILE`, and `HF_HOME` through environment variables and uses a literal heredoc so Python f-strings are not shell-expanded.
- **Follow-up fix:** A later rerun correctly waited for dpkg/archive locks while unattended-upgrade built kernel/DKMS artifacts, but then failed on `E: Could not get lock /var/lib/apt/lists/lock`. The bootstrap/deploy apt-lock wait helpers now include `/var/lib/apt/lists/lock` as well.
- **Follow-up fix:** The first SGLang launch then crash-looped before binding `:8001` with `ImportError: cannot import name 'dynamic_per_tensor_quant' from 'aiter'` followed by `ImportError: aiter is required when SGLANG_USE_AITER is set to True`. After setting `SGLANG_USE_AITER=0` only in the launcher, the image still crashed while importing AMD Quark quantization with `AttributeError: module 'aiter' has no attribute 'dtypes'`. The ROCm launcher and the locally built image now pin `SGLANG_USE_AITER=0`, `SGLANG_ROCM_DISABLE_LINEARQUANT=1`, and `SGLANG_ENABLE_QUARK_QUANTIZATION=0`, and deploy builds `clypt/sglang:v0.5.10-rocm720-mi30x-clypt1` from the pinned upstream image with Quark quantization removed from the default registry. `Qwen/Qwen3.6-35B-A3B` is not a Quark checkpoint, so the patched image remains fail-fast if Quark is explicitly enabled or unexpectedly appears in `QUANTIZATION_METHODS`.
- **Follow-up fix:** The staged profile drop-in initially did not override `SG_LAUNCH_PROFILE=final` from `/etc/clypt-phase26/phase26.env`, because systemd `EnvironmentFile` values can override earlier `Environment=` assignments. The deploy drop-in now writes `SG_LAUNCH_PROFILE_OVERRIDE`, and the launcher gives that value precedence for staged validation.
- **Follow-up fix:** After the image started importing successfully, SGLang rejected the H200 Mamba flag with `AssertionError: Mamba extra_buffer is only supported on CUDA devices with FLA backend`. AMD now uses `SG_MAMBA_SCHEDULER_STRATEGY=no_buffer`; speculative profiles add `--disable-radix-cache` because SGLang rejects Qwen3.6 `no_buffer + speculative + radix cache`.
- **Follow-up fix:** Minimal-profile startup then resolved the model architecture to `Qwen3_5MoeForConditionalGeneration` but reported no compatible SGLang implementation because the `qwen3_5` model module import was skipped after `ep_moe/layer.py` imported Quark MoE schemes, which hit the same incompatible AITER `dtypes` path. The local Clypt SGLang image now also guards the `QuarkW4A4MXFp4MoE` import behind `SGLANG_ENABLE_QUARK_QUANTIZATION` and installs a dummy class for non-Quark checkpoints.
- **Follow-up fix:** Once minimal boot passed, profile cycling hung on `systemctl stop` and timed out after SGLang had returned `/v1/models`; the unit entered `failed (Result: timeout)` and the deploy loop kept polling a dead endpoint. The SGLang unit now has `ExecStop=-/usr/bin/docker rm -f clypt-phase26-sglang-qwen` and `TimeoutStopSec=30`, and deploy stops/removes the container plus resets failed unit state before each staged start.
- **Follow-up fix:** A later live deploy had two concurrent `deploy_phase26_mi300x_services.sh` processes on the same droplet, so one validation run stopped the SGLang unit immediately after another run observed `/v1/models` as healthy. The symptom was repeated `curl: (7) Failed to connect to 127.0.0.1 port 8001` while the unit sat in `failed (status=137)` and the profile drop-in still showed the previous stage. The deploy script now takes `/var/lock/clypt-phase26-mi300x-deploy.lock` with `flock -n` and fails fast if another MI300X deploy is already active.
- **Follow-up fix:** The final SGLang profile, LocalOpenAI smoke, and long-context smoke passed, but the deploy failed one step later because it curled `http://127.0.0.1:9300/health` immediately after `systemctl restart clypt-phase26-dispatch.service`; Uvicorn bound the port about one second later. The deploy script now waits for Phase26 dispatch and Phase1 API health with a retry loop instead of treating a sub-second readiness race as a hard failure.
- **Follow-up fix:** The Phase1 API runbooks and deploy script expected `GET /health`, but the actual FastAPI app only exposed `GET /healthz`, so the live service was running while the deploy loop saw repeated `404 Not Found` on `/health`. The app now exposes `/health` as an alias while preserving `/healthz`.
...
- **Subsystem:** Phase1 MI300X live smoke (`run_phase1`, VibeVoice vLLM ROCm, RF-DETR PyTorch ROCm, AMD VAAPI decode)
- **Environment:** DigitalOcean `clypt-phase1-mi300x-atl1` (`134.199.206.56`), test-bank URL `https://youtu.be/64qBE35S0ek?si=bul2StVGVzUE8EL6`, mapped asset `joeroganflagrant.mp4` / `joeroganflagrant.wav`
- **Symptom / Error signature:** Phase1-only smoke launched visual extraction and VibeVoice ASR in parallel. VibeVoice repeatedly failed with `finish_reason=length`, invalid JSON (`Unterminated string`), and persisted raw artifacts around `88.4k` chars. Artifact analysis showed the same historical degeneration class as the 2026-04-15 `joeroganflagrant.wav` issue, now repeating `clack-clack-clack` about `4k` three-gram repeats. Raising live `VIBEVOICE_REPETITION_PENALTY` to `1.05` did not fix it; follow-up attempts failed after about `205-206s` with the same signature. RF-DETR also failed to provide a successful visual payload during the combined smoke: with implicit ROCm `torch.compile`, the visual service stayed in warmup/trace for about `19m` without `Warmup complete`; with compile disabled, the combined VibeVoice+visual run was stopped before visual completion because ASR had already entered the repetition loop.
- **Root cause:** Two separate findings. For ASR, the clip still triggers VibeVoice token-loop degeneration; a small repetition-penalty bump is insufficient and repeated identical retries waste GPU time. For visual, `torch.compile` is not currently a viable MI300X default for RF-DETR warmup, but the post-compile-disable performance gap was initially misattributed to RF-DETR itself. Follow-up isolation showed RF-DETR/VAAPI/ByteTrack are fast when ASR is not actively generating; the combined run was blocked by VibeVoice failure and likely GPU contention during the long looping ASR request, not by baseline RF-DETR throughput.
- **Fix applied:** Added `CLYPT_PHASE1_VISUAL_TORCH_COMPILE=0` as the MI300X default and wired RF-DETR to use that explicit knob instead of compiling whenever TensorRT is absent. Synced the change to the live host and restarted the visual service. Set live `VIBEVOICE_REPETITION_PENALTY=1.05` for the requested experiment and updated the MI300X baseline env, but this did not resolve the repetition failure.
- **Verification evidence:** Live env on `134.199.206.56` confirmed Nano, batch `16`, threshold `0.35`, shape `640`, VAAPI decode, ROCm FP16 detector, and `CLYPT_PHASE1_VISUAL_TORCH_COMPILE=0`. Detector-only probes with services paused reported batch `16` RF-DETR at about `464.5 fps` after warmup (`mean_detector_latency_ms=2.2`). Decode-only probes on `joeroganflagrant.mp4` reached `292-815 fps` over `128-2048` frames. Full visual-only extraction on the same asset completed in `82.25s` wall time; tracking metrics reported `18,908` frames processed, `340.8` effective FPS, `55,484.4 ms` pipeline elapsed, `10,467.9 ms` warmup, and `62,636` emitted track rows. VibeVoice reruns at `VIBEVOICE_REPETITION_PENALTY=1.05` still failed with `finish_reason=length generated_chars=88397 approx_generated_tokens=22099`, including artifacts `vibevoice_1777704241861_307058.txt` and `vibevoice_1777704446525_307058.txt`.
- **Follow-up fix:** Patched `VibeVoiceVLLMProvider` to detect repetition loops while streaming, truncate recovery state to the last complete JSON segment boundary, and retry with an assistant prefix plus the upstream VibeVoice recovery sampling profile (`temperature=0.2`, then `0.3`, then `0.4`; `top_p=0.95`). This keeps contract/parse failures fail-fast, but avoids waiting for the model to exhaust `max_tokens` when a loop is already visible in the stream.
- **Follow-up verification evidence:** Live Phase1-only rerun `phase1_mi300x_64qbe35s0ek_recovery_20260502_071357` on `134.199.206.56` completed successfully. VibeVoice detected the `clack-clack-clack` loop twice at `chars=12671`, retained a `12,193` char safe prefix, recovered on attempt `2/3`, and returned HTTP `200` with `202` turns in `65,243.9 ms` for `788.689s` audio. Phase1 stage metrics in Spanner show `phase1_forced_alignment` succeeded in `34,648.0 ms` with `2,963` words, `phase1_emotion2vec` succeeded in `35,030.2 ms`, `phase1_yamnet` succeeded in `3,939.4 ms` with `48` events, and `phase1_visual_extraction` succeeded in `186,884.5 ms` with `62,632` tracks and `127` shot changes while sharing the MI300X with VibeVoice/audio-post.
- **Follow-up guardrails:** Treat VibeVoice repetition as a distinct retry class, not a generic transient HTTP 500: detect rolling n-gram loops while streaming, persist the artifact, and retry with the upstream auto-recovery profile or a shorter sharding profile instead of repeating the same greedy request. Keep ROCm `torch.compile` opt-in only. If visual regresses again, isolate detector-only, decode-only, visual-only, and concurrent-ASR cases before attributing the issue to RF-DETR.

## 2026-05-02 - Rithvik-AMD MI300X Phase1 provisioning requires devcloud size/image path

- **Date/Time (UTC):** 2026-05-02 05:27 UTC
- **Subsystem:** DigitalOcean Phase1 MI300X provisioning (`doctl`, Rithvik-AMD team, ROCm 7.2 application image, `gpu-mi300x1-192gb-devcloud`)
- **Environment:** Local operator machine on `AMD-refactor`, active `doctl` context `rithvik-amd`, project `Clypt` (`75263152-7331-4244-a367-e76a55fcae89`)
- **Symptom / Error signature:** `doctl compute size list` reported the public `gpu-mi300x1-192gb` slug as `available=true`, and `gpu-amd-base` existed as image `221919586`, but CLI creation failed in every image-supported region with `422 Size is not available in this region` or equivalent region-unavailable errors. The DigitalOcean UI could still create a working MI300X droplet using the hidden/devcloud size `gpu-mi300x1-192gb-devcloud` and ROCm 7.2 application image.
- **Root cause:** The repository assumed the generic public `gpu-mi300x1-192gb` + `gpu-amd-base` path was the source-of-truth provisioning path. For this team, the actual provisionable MI300X path is `gpu-mi300x1-192gb-devcloud` in `atl1` with image slug `amddevelopercloud-rocm72software` (`ROCm™ 7.2 Software 7.2 on Ubuntu 24.04`).
- **Fix applied:** Adopted the UI-created droplet `568498677`, renamed it to `clypt-phase1-mi300x-atl1`, tagged it with `clypt-phase1-mi300x` and `amd-refactor`, set the host hostname to match, and updated the deploy docs/specs/bootstrap diagnostics to prefer the ROCm 7.2 MI300X application image path.
- **Verification evidence:** `doctl compute droplet get 568498677` showed `clypt-phase1-mi300x-atl1`, public IP `134.199.206.56`, private IP `10.128.0.2`, region `atl1`, size `gpu-mi300x1-192gb-devcloud`, image `ROCm™ 7.2 Software 7.2 on Ubuntu 24.04`, and tags `amd-refactor,clypt-phase1-mi300x`. SSH host checks showed `/dev/kfd`, `/dev/dri/renderD*`, `rocm-smi`, `amd-smi`, Docker, FFmpeg, and MI300X gfx942 VRAM visible. The first bootstrap attempt exposed that this image already installs `docker-ce` and `containerd.io`; the bootstrap/deploy scripts now skip `docker.io` when Docker is already present to avoid the `containerd.io : Conflicts: containerd` apt resolver failure. The first deploy attempt also raced `unattended-upgr` on `/var/lib/dpkg/lock-frontend`; AMD bootstrap/deploy scripts now wait for apt/dpkg locks before `apt-get update` and install. The next deploy attempt proved PyTorch ROCm on MI300X (`torch.version.hip=6.2.41134-65d174c3e`, BF16 matmul) and then exposed that `/dev/dri/renderD128` is a virtio node while `/dev/dri/renderD129` is the AMD VAAPI node; `discover_vaapi_render_node()` now validates render nodes with `vainfo` and chooses the first passing node instead of blindly picking the lowest render number.
- **Follow-up fix:** Phase1 and Phase26 envs keep `HF_HUB_OFFLINE=1` for steady-state runtime, but deploy-time model prewarm must run online. The MI300X deploy scripts now override `HF_HUB_OFFLINE=0` only around Hugging Face `model_info` / `snapshot_download` prewarm calls before returning services to offline mode.
- **Follow-up fix:** The VibeVoice snapshot prewarm also writes `/etc/clypt-phase1/vibevoice-model.env`; `VIBEVOICE_MODEL_ENV_FILE` is now explicitly passed into that Python subprocess so the deploy can write the resolved offline snapshot path after download.
- **Follow-up fix:** Live RF-DETR prewarm exposed that `rfdetr 1.6.5.post0` rewrites Nano/Small `positional_encoding_size` when `resolution=640` is passed, causing checkpoint load to fail with `size mismatch for backbone.0.encoder.encoder.embeddings.position_embeddings`. The AMD RF-DETR loader now preserves the configured runtime resolution while passing the checkpoint-native PE grid. Detection-only Nano used a `24` grid; live RF-DETR-Seg Nano requires `26` because its checkpoint position embedding has `677 = 26 * 26 + 1` rows. Seg Nano also requires the runtime shape to be divisible by patch size `12`, so the active Modal shape is `648`.
- **Follow-up fix:** The audio-post prewarm also needs an online Hugging Face window. With `HF_HUB_OFFLINE=1`, FunASR could not populate `emotion2vec/emotion2vec_plus_large`, then failed registration for `iic/emotion2vec_plus_large`. The MI300X deploy now overrides `HF_HUB_OFFLINE=0` for the NFA/emotion2vec+/YAMNet prewarm block while keeping offline mode for steady-state services after caches are populated.
- **Follow-up fix:** The VibeVoice vLLM plugin also generates tokenizer files from `Qwen/Qwen2.5-7B`. Letting this happen only at systemd startup failed under `HF_HUB_OFFLINE=1` with `OfflineModeIsEnabled` and then vLLM crashed with `TypeError: expected str, bytes or os.PathLike object, not NoneType`. The deploy now runs `vllm_plugin.tools.generate_tokenizer_files` inside the built ROCm image with `HF_HUB_OFFLINE=0`, mounted against the same HF cache and resolved `VIBEVOICE_MODEL_PATH`, then the sidecar startup runs the same command fail-hard under offline mode instead of hiding failures with `|| true`.
- **Final verification evidence:** The corrected Phase1 MI300X deploy completed on `134.199.206.56`; `clypt-phase1-vllm-vibevoice`, `clypt-phase1-vibevoice`, `clypt-phase1-visual`, `clypt-phase1-api`, and `clypt-phase1-worker` were all `active`. The local vLLM sidecar answered `/v1/models` as `vibevoice` with `max_model_len=131072`; Phase1 local health endpoints on `8080`, `9100`, and `9200` returned healthy JSON; Docker was left with only the VibeVoice vLLM container running; `rocm-smi` reported the MI300X VF (`gfx942`) with the warmed VibeVoice sidecar using GPU memory.
...
- **Subsystem:** Phase1 visual runtime (`backend/phase1_runtime/*`, Phase1 host runtime env/docs)
- **Environment:** Current production Phase1 H200 on `107.170.11.76`
- **Symptom / Error signature:** The experimental zero-copy / StreamReader overlap path introduced in the visual optimization pass was not production-ready: it first failed on torio EOF handling, then after the EOF fix it still failed the production bar because the long visual-only smoke no longer completed in the previously known-good time window. The user requested a rollback to the last known-good pre-optimization visual path before doing any further detector retuning.
- **Root cause:** The optimization pass changed both the decode substrate and the scheduling model at once, which made it hard to separate stability regressions from throughput regressions. Even after the EOF failure was understood, the optimized path was still not a validated replacement for the prior ffmpeg NVDEC pipeline.
- **Fix applied:** Restored the Phase1 visual runtime files to the pre-`8ff12cd` implementation (ffmpeg NVDEC decode feeding the existing RF-DETR + ByteTrack path), kept the standalone `scripts/tmp_rfdetr_visual_only.py` helper for host validation, and made the one intended runtime change on top of that stable baseline: Phase1 now defaults to `CLYPT_PHASE1_VISUAL_MODEL=nano`, which selects RF-DETR Nano instead of Small in both the PyTorch and TensorRT detector paths. Updated the committed Phase1 env baseline and deployment/runtime docs to make the model selection explicit.
- **Verification evidence:** Focused visual/runtime tests passed after the rollback-plus-Nano change: `./.venv/bin/python -m pytest tests/backend/phase1_runtime/test_rfdetr_visual_modules.py tests/backend/phase1_runtime/test_media_and_visual.py tests/backend/runtime/phase1_visual_service/test_phase1_visual_service_app.py tests/backend/providers/test_phase1_visual_and_phase26_clients.py -q`.
- **Follow-up guardrails:** Keep future visual speed experiments behind a clearly separate branch/spec until they beat the known-good ffmpeg NVDEC baseline in a complete host-side smoke. Treat `CLYPT_PHASE1_VISUAL_MODEL` as the sanctioned knob for future Small/Nano retunes; avoid baking the model size into hidden code-only defaults again.

## 2026-04-21 - Temp visual-only runner schema mismatch and fresh-host Nano engine build tax

- **Date/Time (UTC):** 2026-04-22 05:15-05:35 UTC
- **Subsystem:** Phase1 visual smoke helper + TensorRT engine bootstrap (`scripts/tmp_rfdetr_visual_only.py`, `backend/phase1_runtime/tensorrt_detector.py`, `scripts/do_phase1/deploy_phase1_services.sh`)
- **Environment:** Production Phase1 H200 on `107.170.11.76`, RF-DETR Nano validation against the canonical `dwarkeshjensen.mp4` test-bank asset
- **Symptom / Error signature:** The first Nano visual-only smoke spent about `15m 03s` wall-clock and then failed in the helper script with `KeyError: 'shot_boundaries'` even though the underlying visual extractor had completed far enough to return a payload. The same run also showed a fresh-host build tax of about `117.9s` from TensorRT engine generation plus an unnecessary `trtexec` inference benchmark.
- **Root cause:** The standalone smoke helper had been written against the experimental visual-optimization payload shape and still expected `shot_boundaries`, while the reverted known-good extractor returns `shot_changes` plus `tracking_metrics.shot_count`. Separately, the TensorRT engine build path delegated to `rfdetr.export.tensorrt.trtexec`, which runs an extra benchmark/inference pass after building the engine, and the deploy/bootstrap path did not explicitly prebuild the active Nano engine on fresh hosts.
- **Fix applied:** Updated the helper to read the reverted production payload contract (`shot_changes` / `tracking_metrics.shot_count`), switched TensorRT engine creation to an explicit build-only `trtexec --skipInference` CLI path that hard-checks the output `.engine`, and added Phase1 deploy-time prewarming of the active RF-DETR TensorRT engine so fresh droplets build it once during bootstrap rather than on the first live visual request.
- **Verification evidence:** Added regression coverage for the helper payload contract and the no-benchmark TensorRT CLI flags, then reran the focused suites: `tests/backend/phase1_runtime/test_rfdetr_visual_modules.py`, `tests/backend/phase1_runtime/test_media_and_visual.py`, `tests/backend/runtime/phase1_visual_service/test_phase1_visual_service_app.py`, `tests/backend/providers/test_phase1_visual_and_phase26_clients.py`, and `tests/scripts/test_tmp_rfdetr_visual_only.py`.
- **Follow-up guardrails:** Treat `backend/outputs/tensorrt_engines/*.engine` as host-local warm cache artifacts; they should be reused whenever the model family, batch size, and input shape are unchanged. Any future smoke helper should be validated against the current production extractor payload schema before being used for operational timing claims.

## 2026-04-21 - Production fix for TorchAudio NVDEC EOF uses process_packet drain loop instead of fill_buffer

- **Date/Time (UTC):** 2026-04-22 04:35-04:50 UTC
- **Subsystem:** Phase1 visual decode (`backend/phase1_runtime/frame_decode.py`, `backend/phase1_runtime/visual.py`, `scripts/tmp_rfdetr_visual_only.py`)
- **Environment:** Live Phase1 H200 validation on `clypt-phase1-h200-rithvik-nyc2` against the canonical `dwarkeshjensen.mp4` test-bank asset and a 20-second trimmed probe clip
- **Symptom / Error signature:** The first two live runs of the new TorchAudio/torio NVDEC path both failed after about `12m 06s` with `RuntimeError: Failed to process a packet. (End of file).`, which bubbled up as `RuntimeError: Phase1 visual decode producer failed.`. The original local hotfix normalized EOF around `fill_buffer()`, but the second live rerun showed the host was still executing the stale nested module because the hotfix rsync command flattened files into the repo root instead of replacing `backend/phase1_runtime/frame_decode.py`.
- **Root cause:** The actual runtime bug is lower than the first patch assumed: on this torio CUDA video path, `fill_buffer()` itself is not a reliable EOF API and raises from the underlying `process_packet()` call instead of returning `1` as documented. A direct host-side reproduction on a 20-second clip showed `fill_buffer()` crashing with the same EOF exception while a manual `process_packet()` + `pop_chunks()` drain loop completed successfully on the same `h264_cuvid` + `hw_accel=cuda:0` stream configuration.
- **Fix applied:** Reworked `decode_video_frame_batches()` to stop using `fill_buffer()` entirely and instead drive the decoder with explicit `process_packet()` calls followed by `pop_chunks()` until EOF plus final buffered-chunk drain. Kept the EOF string-match normalization only as a last-resort guard around `process_packet()` for runtime-dependent torio behavior. Added regression coverage proving that the decode loop (1) uses `process_packet()`, (2) drains the final chunk after `process_packet()` reports EOF, and (3) still raises non-EOF decode errors.
- **Verification evidence:** Live host reproduction on `/tmp/dwarkesh20s.mp4` showed `fill_buffer()` failing in `3.43s` with `RuntimeError: Failed to process a packet. (End of file).` while the equivalent `process_packet()` drain loop completed successfully in `3.07s` with `597` chunks / `601` frames. Focused local regression tests passed after the code change: `./.venv/bin/python -m pytest tests/backend/phase1_runtime/test_rfdetr_visual_modules.py -q`.
- **Follow-up guardrails:** Treat `StreamReader.fill_buffer()` as unsafe on the current TorchAudio/torio NVDEC stack for production visual extraction. Any future Phase1 visual decode changes must be validated on-host with the visual-only runner, and sync commands must preserve nested repo paths (`rsync -R` or full-tree sync) so a “tested” hotfix is actually the one running on the droplet.

## 2026-04-21 - New RF-DETR TorchAudio NVDEC path treated torio EOF as a hard failure

- **Date/Time (UTC):** 2026-04-22 04:05-04:17 UTC
- **Subsystem:** Phase1 visual decode (`backend/phase1_runtime/frame_decode.py`, `backend/phase1_runtime/visual.py`, `scripts/tmp_rfdetr_visual_only.py`)
- **Environment:** Live Phase1 H200 visual-only smoke on `clypt-phase1-h200-rithvik-nyc2` against the canonical `dwarkeshjensen.mp4` test-bank asset
- **Symptom / Error signature:** The first live run of the new TorchAudio/torio NVDEC visual path stayed alive for about `11m 42s` and then failed at stream completion with `RuntimeError: Failed to process a packet. (End of file).`, which bubbled up as `RuntimeError: Phase1 visual decode producer failed.` from the producer thread in `_run_rfdetr_tracking_pipeline()`.
- **Root cause:** The new `decode_video_frame_batches()` loop assumed `StreamReader.fill_buffer()` would report end-of-stream via a normal status code. On the live H200 path, torio instead raised a `RuntimeError` containing `End of file` at EOF. The producer treated that as a real decoder failure rather than a clean stream completion signal.
- **Fix applied:** Added `_is_streamreader_eof_error()` in `frame_decode.py` and normalized the torio `End of file` runtime error into `status = 1` so the decode loop exits cleanly after draining any remaining chunk. Added regression tests covering both EOF normalization and non-EOF decoder errors in `tests/backend/phase1_runtime/test_rfdetr_visual_modules.py`.
- **Verification evidence:** Focused Phase1 visual module tests now pass with the new EOF coverage: `./.venv/bin/python -m pytest tests/backend/phase1_runtime/test_rfdetr_visual_modules.py -q`.
- **Follow-up guardrails:** Treat torio/FFmpeg EOF behavior as runtime-dependent. Any future changes to the direct-CUDA decode path should be live-smoked on the H200 host with the visual-only runner before assuming a local unit-tested path is production-safe.
...
- **Subsystem:** Phase26 remote node-media-prep caller + Modal app (`backend/providers/node_media_prep_client.py`, `scripts/modal/node_media_prep_app.py`)
- **Environment:** Ming-team Phase26 H200 (`clypt-phase26-h200-ming-nyc2`) calling the live Modal L4 app `clypt-node-media-prep` during long-form replay `job_8abec2b39a0b4a1eb5c61d654477436a`
- **Symptom / Error signature:** Phase1 succeeded, Phase2 invoked node-media-prep for `65` nodes, then failed after about `761s` with `RemoteNodeMediaPrepError: node-media-prep /tasks/node-media-prep returned HTTP 303: The HTTP server returned a redirect error that would lead to an infinite loop. The last 30x error message was: See Other`.
- **Root cause:** The original Modal app performed the entire clip extraction job inside the web request. Once runtime crossed Modal's `150s` webhook limit, Modal began returning chained `303 See Other` result redirects. The Phase26 client used Python `urllib`, which follows only a small number of same-URL redirects by default, so longer node-media-prep calls tripped the redirect cap and failed even though the underlying Modal work could keep running.
- **Fix applied:** Refactored node-media-prep to an explicit submit-and-poll contract. [`scripts/modal/node_media_prep_app.py`](../scripts/modal/node_media_prep_app.py) now returns `202 Accepted` with a `call_id` from `POST /tasks/node-media-prep`, exposes `GET /tasks/node-media-prep/result/{call_id}`, and runs the heavy ffmpeg/GCS job in a spawned Modal function `node_media_prep_job`. [`backend/providers/node_media_prep_client.py`](../backend/providers/node_media_prep_client.py) now submits once, polls the result endpoint until completion within `CLYPT_PHASE24_NODE_MEDIA_PREP_TIMEOUT_S`, and preserves the existing final ordered `media` list contract for Phase26. Updated runtime/deploy docs and added focused regression coverage for both the client and Modal app.
- **Verification evidence:** Local focused suites passed after the refactor: `./.venv/bin/python -m pytest tests/backend/providers/test_remote_node_media_prep_client.py -q` (`13 passed`) and `./.venv/bin/python -m pytest tests/scripts/test_modal_node_media_prep_app.py -q` (`6 passed`). The traced failure window also matched Modal's documented webhook timeout behavior almost exactly (`~761s`, i.e. five `150s` redirect windows plus overhead), confirming the root cause before patching.
- **Follow-up guardrails:** Keep long-running Modal work off the web request path. If future Phase 6 render/export jobs can cross Modal's webhook timeout boundary, use the same submit-and-poll pattern instead of relying on chained `303` redirects.

## 2026-04-20 - Forced aligner global OOM on long transcripts now routes through duration-chunked global alignment, not per-turn fallback

- **Date/Time (UTC):** 2026-04-20 04:35 UTC
- **Subsystem:** Phase1 forced alignment (`backend/providers/forced_aligner.py`)
- **Environment:** Rithvik-team Phase1 H200 during long-form Phase1 job `job_8abec2b39a0b4a1eb5c61d654477436a`
- **Symptom / Error signature:** Global forced alignment on a `103.2` minute source with `371` turns failed with `OutOfMemoryError: CUDA out of memory. Tried to allocate 178.57 GiB`, after which the old runtime fell back to per-turn alignment. That fallback completed, but it is materially less stable than aligning in larger transcript sections.
- **Root cause:** The old forced aligner attempted one full global pass regardless of transcript size, then jumped directly to per-turn slices on any failure. For long transcripts, the global pass can exceed H200 VRAM, and the per-turn fallback over-fragments context and increases alignment error risk.
- **Fix applied:** Replaced the per-turn fallback policy with duration-based default chunked global alignment in [`backend/providers/forced_aligner.py`](../backend/providers/forced_aligner.py). The new defaults are: `<=40 min` -> `1` chunk, `>40..60 min` -> `2`, `>60..120 min` -> `3`, `>120..150 min` -> `4`, and `>150 min` -> `5`. Each chunk aligns contiguous full turns against a sliced audio window, then words are re-offset into the global timeline. If any chunk-level alignment fails, the provider now hard-fails instead of silently dropping to per-turn alignment. Added regression coverage in [`tests/backend/providers/test_forced_aligner.py`](../tests/backend/providers/test_forced_aligner.py).
- **Verification evidence:** Local focused tests passed after the patch: `./.venv/bin/python -m pytest tests/backend/providers/test_forced_aligner.py -q` (`3 passed`). The runtime guide was updated to record the new load-bearing default chunk policy.
- **Follow-up guardrails:** Do not reintroduce per-turn forced-alignment fallback without explicit approval; it hides OOM pressure by trading memory safety for worse alignment quality. Any future tuning to NFA chunk thresholds must update code, runtime docs, and this log in the same task.

## 2026-04-19 - Long-form VibeVoice speaker verifier passed a path string into SpeechBrain `encode_batch`

- **Date/Time (UTC):** 2026-04-20 03:40 UTC
- **Subsystem:** Phase1 long-form VibeVoice speaker stitching (`backend/providers/speaker_verifier.py`, `backend/runtime/phase1_vibevoice_service/speaker_stitch.py`)
- **Environment:** Rithvik-team Phase1 H200 (`clypt-phase1-h200-rithvik-nyc2`) on long-form test-bank run `job_71353e3e38114b81b3eb37edd60edb38` for `dwarkeshjensen.mp4`
- **Symptom / Error signature:** Phase1 failed during VibeVoice ASR with repeated `HTTP 500` responses carrying `VibeVoice ASR failed: 'str' object has no attribute 'shape'`. The Phase1 VibeVoice service traceback pointed at SpeechBrain `EncoderClassifier.encode_batch(...)` after the long-form shard ASR completed and speaker stitching began.
- **Root cause:** The ECAPA-TDNN wrapper called `classifier.encode_batch(str(audio_path))`, but SpeechBrain `encode_batch(...)` expects waveform tensors, not filesystem paths. The shorter single-pass test-bank video never hit this bug because only the long-form path invokes cross-shard speaker verification during `stitch_global_speakers(...)`.
- **Fix applied:** Patched [`backend/providers/speaker_verifier.py`](../backend/providers/speaker_verifier.py) to use SpeechBrain’s own `load_audio(...)` helper, build a one-item batch plus relative lengths tensor, and then call `encode_batch(...)` with waveform data instead of a path string. Added a regression test in [`tests/backend/providers/test_speaker_verifier.py`](../tests/backend/providers/test_speaker_verifier.py) that fails if the verifier skips audio loading and passes a raw path into `encode_batch(...)`.
- **Verification evidence:** Reproduced the live traceback from `journalctl -u clypt-phase1-vibevoice.service` on the H200 host, confirming the failing line was `embedding = classifier.encode_batch(str(audio_path))`. After the local patch, the new regression test passes along with the focused long-form/runtime suites: `./.venv/bin/python -m pytest tests/backend/providers/test_speaker_verifier.py tests/backend/runtime/phase1_vibevoice_service/test_longform.py -q` (`10 passed`) and `./.venv/bin/python -m pytest tests/backend/runtime/phase1_vibevoice_service/test_app.py tests/backend/providers/test_provider_config_and_clients.py -q` (`31 passed`).
- **Follow-up guardrails:** Treat SpeechBrain path-based helpers (`load_audio`, `classify_file`) and tensor-based helpers (`encode_batch`) as distinct APIs. Long-form-only code paths need dedicated regression tests even when the short-form Phase1 path is already green, because shard stitching introduces verifier-specific behavior that single-pass runs never exercise.

## 2026-04-19 - Phase26 node-media-prep client double-appended the Modal task path

- **Date/Time (UTC):** 2026-04-20 01:55 UTC
- **Subsystem:** Phase26 remote node-media-prep caller (`backend/providers/node_media_prep_client.py`)
- **Environment:** Fresh Ming-team Phase26 H200 (`clypt-phase26-h200-ming-nyc2`) calling the live Modal L4 app `clypt-node-media-prep`
- **Symptom / Error signature:** Clean Phase 1 handoff succeeded, then Phase 2 failed almost immediately with `RemoteNodeMediaPrepError: node-media-prep /tasks/node-media-prep returned HTTP 404: {"detail":"Not Found"}`.
- **Root cause:** `CLYPT_PHASE24_NODE_MEDIA_PREP_URL` on the live host was set to the full Modal task endpoint (`.../tasks/node-media-prep`), but `RemoteNodeMediaPrepClient._endpoint()` always appended `/tasks/node-media-prep` again. The live request therefore hit `POST /tasks/node-media-prep/tasks/node-media-prep`, which Modal correctly returned as `404 Not Found`.
- **Fix applied:** Patched [`backend/providers/node_media_prep_client.py`](../backend/providers/node_media_prep_client.py) so the client accepts either a Modal base URL or a full task endpoint URL and does not double-append the task path. Added a focused regression test in [`tests/backend/providers/test_remote_node_media_prep_client.py`](../tests/backend/providers/test_remote_node_media_prep_client.py), updated the Phase26 env/deploy records to the current Modal hostname, clarified the accepted URL shapes in the runtime/deploy docs, and added an `AGENTS.md` guardrail requiring live deploy fixes to be reflected in local source-of-truth files during the same task. Synced the patched client onto the live Phase26 host and restarted `clypt-phase26-worker.service`.
- **Verification evidence:** Modal app logs showed the bad live request exactly as `POST /tasks/node-media-prep/tasks/node-media-prep -> 404 Not Found`. Direct probes to the deployed app showed the intended route existed (`POST /tasks/node-media-prep -> 401 Unauthorized` without the bearer token), proving the failure was caller-side. The focused provider suite passed locally after the patch: `./.venv/bin/python -m pytest tests/backend/providers/test_remote_node_media_prep_client.py tests/backend/providers/test_provider_config_and_clients.py -q` (`40 passed`). The live Phase26 host file hash for `backend/providers/node_media_prep_client.py` matches local after the restart.
- **Follow-up guardrails:** Treat `CLYPT_PHASE24_NODE_MEDIA_PREP_URL` as an operator-facing endpoint setting that may be recorded either as the Modal base URL or the full task route. Do not assume one shape in callers without normalizing it, and whenever a live deploy uncovers a drift like this, update the repo’s env records/docs/tests before considering the fix complete.
...
## 2026-04-20 - Phase1 forced aligner prewarm used the wrong cache home on fresh H200 hosts

- **Date/Time (UTC):** 2026-04-20 01:42-01:44
- **Subsystem:** Phase1 H200 deploy / forced alignment cache prewarm
- **Environment:** Fresh Phase1 host `clypt-phase1-h200-rithvik-nyc2`, live test-bank replay `job_7f86a518067e4b4e9b7c582a0459a1e8`
- **Symptom / Error signature:** Phase1 VibeVoice ASR completed, then forced alignment failed with `ValueError: Not able to download url right now, please try again.` while trying to load `stt_en_fastconformer_hybrid_large_pc` from `/opt/clypt-phase1/.cache/torch/NeMo/...`.
- **Root cause:** `deploy_phase1_services.sh` prewarmed the NeMo forced-aligner model under root's default cache (`/root/.cache/torch/NeMo/...`), but the running Phase1 services use `HOME=/opt/clypt-phase1`, `CLYPT_PHASE1_CACHE_HOME=/opt/clypt-phase1/.cache`, and `TORCH_HOME=/opt/clypt-phase1/.cache/torch`. The live worker therefore missed the prewarmed artifact and attempted a runtime redownload into the service cache path.
- **Fix applied:** Seeded `/opt/clypt-phase1/.cache/torch/NeMo/.../stt_en_fastconformer_hybrid_large_pc.nemo` from the already-downloaded `/root/.cache/torch/NeMo/...` artifact on the live host, then requeued the affected test-bank run.
- **Verification evidence:** Confirmed the source `.nemo` existed only in `/root/.cache/...` before the fix; confirmed the destination `.nemo` existed in `/opt/clypt-phase1/.cache/...` after the copy; observed the original run log reach VibeVoice success and fail exactly at forced-aligner model load; and queued a clean retry (`job_641469d29f3241d689a9c7cb7dd6f5bc`) after seeding the correct cache path.
- **Follow-up guardrails:** Keep Phase1 prewarm steps aligned with the runtime service env (`HOME`, `XDG_CACHE_HOME`, `TORCH_HOME`, `HF_HOME`) on fresh hosts; do not treat root-owned prewarm caches as sufficient verification for systemd-managed service startup.

## 2026-04-20 - Phase1 H200 steady-state vLLM reservation left insufficient headroom for global forced alignment

- **Date/Time (UTC):** 2026-04-20 01:56-02:10
- **Subsystem:** Phase1 H200 VibeVoice vLLM sidecar / forced aligner GPU sharing
- **Environment:** Fresh Phase1 host `clypt-phase1-h200-rithvik-nyc2`, live test-bank replay `job_5f4a420eef3a4d8792aef6401d9765fb`
- **Symptom / Error signature:** `OutOfMemoryError: CUDA out of memory. Tried to allocate 10.38 GiB` during global forced alignment on a roughly 18 minute test-bank clip, followed by automatic fallback to per-turn alignment.
- **Root cause:** The hot VibeVoice sidecar was using about `116.51 GiB` of the H200 under the H200 baseline `VIBEVOICE_VLLM_GPU_MEMORY_UTILIZATION=0.82` and `VIBEVOICE_VLLM_MAX_NUM_SEQS=4`, leaving only about `8.48 GiB` free when the forced aligner requested another `10.38 GiB`. The failure was caused by steady-state GPU budget pressure, not by the clip length alone.
- **Fix applied:** Lowered the Phase1 H200 baseline twice while tuning against the live replay: first to `VIBEVOICE_VLLM_GPU_MEMORY_UTILIZATION=0.75`, then further to `0.65`, while keeping `VIBEVOICE_VLLM_MAX_NUM_SEQS=3`. Mirrored the same env change onto the live Phase1 host before restarting the sidecar.
- **Verification evidence:** OOM-era journal showed the allocation split (`116.51 GiB` vLLM engine, `1.55 GiB` visual service, `12.52 GiB` Phase1 worker, `8.48 GiB` free). After the env change, the sidecar was restarted and re-polled for health and model readiness before relaunching the same test-bank input as a true `run_phase14` job.
- **Follow-up guardrails:** Treat H200 sidecar memory reservation and max seq count as a shared-GPU headroom budget with the forced aligner and visual service, not as an isolated VibeVoice throughput knob.
